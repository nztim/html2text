...
class Html2Text
{
	 ...
	 */
	public static function convert($html, array $options = []): string
	{
	...
	private static function nbspCodes(): array
	{
		return [
			"\xc2\xa0",
			"\u00a0",
	...
	private static function zwnjCodes(): array
	{
		return [
			"\xe2\x80\x8c",
			"\u200c",
	 ...
	 * @return string the fixed text
	 */
	private static function processWhitespaceNewlines($text): string
	{
	 ...
	 */
	private static function getDocument(string $html, bool $ignore_error = false): DOMDocument
	{
		$doc = new DOMDocument();
		$html = trim($html);
		if (!$html) {
	 ...
	 */
	private static function isOfficeDocument($html): bool
	{
	 ...
	 * by a browser.
	 */
	private static function renderText($text): string
	{
		$text = str_replace(static::nbspCodes(), " ", $text);
		return str_replace(static::zwnjCodes(), "", $text);
	}

	private static function isWhitespace($text): bool
	{
	...
	private static function iterateOverNode($node, $prevName, $in_pre, $is_office_document, $options)
	{
